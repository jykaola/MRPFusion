...
def eval_multi_method(eval_flag=None, dataroot=None, results_root=None, dataset=None, easy_flag=False):
    ...
    if eval_flag is False:   # 如果是正式评估，需要 eval_flag=True
        file_list = []
        for i in range(150):     # 42张图太久了，只要前20张
...
if __name__ == '__main__':
    ...
    #               'U2Fusion', 'FLFuse', 'SeAFusion', 'PIAFusion', 'OwnFusion']
    # algorithms = ["原","无AFM add","无AFM cat","无AFM max"]
    algorithms = ["MRPFusion"]
    # 需要使用的数据集列表
    # datasets = ['TNO', 'RoadScene', 'MSRS', 'M3FD']  # 'TNO', 'RoadScene', 'MSRS', 'M3FD'
    # datasets = ['TNO','MSRS','M3FD']
    datasets = ['MSRS'];
    dataroot = r'D:\project\evalueation\dataset'  # 数据集路径
    results_root = r'D:\project\evalueation\result'  # 算法结果路径

    log_path = r"D:\project\pythonTh_poject\MRPFusion\ECB\logs"

    logger = logging.getLogger()  # 设置日志记录器，用于记录评估过程中的信息
    setup_logger(log_path)

    # 定义保存结果的文件夹路径
    save_folder = r'.\table'  # 新生成Excel文件的保存路径
    ...
    for i in range(len(datasets)):
        dataset = datasets[i]
        logger.info('Dataset: {}'.format(dataset))  # 记录当前数据集

        # 初始化一个空的DataFrame来存储结果
        results_df = pd.DataFrame(columns=['Algorithm', 'EN', 'MI', 'SF', 'AG', 'SD', 'CC', 'SCD',
        ...
        # 生成唯一的Excel文件名，保存在指定文件夹中

        excel_filename = f"{dataset}_evaluation_results_{timestamp}.xlsx"
        ...
        # 如果你仍然想在日志中打印表格，可以继续使用PrettyTable
        table = PrettyTable(results_df.columns.tolist())
        for row in results_df.values:
            formatted_row = [f"{item:.4f}" if isinstance(item, (float, int)) else str(item) for item in row]
